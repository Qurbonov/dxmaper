...
const Ministry = () => {
  ...
  const getIndividualInfo = () => {
    ...
    axios
      .get(
        process.env.REACT_APP_LOCAL_URL_GET_INDIVIDUAL_BY_PINFL +
          `?pinfl=${pinfl}`,
        { headers }
      )
      .then((response) => {
  ...
  const getYuridikInfo = () => {
    ...
    axios
      .get(
        process.env.REACT_APP_LOCAL_URL_GET_YURIDIK_BY_STIR + `?inn=${stir}`,
        { headers }
      )
      .then((response) => {
  ...
  const get_report_year_quarter = () => {
    ...
    axios
      .get(
        process.env.REACT_APP_LOCAL_URL_GET_REPORT_YEAR_QUARTER +
            ...
            <tr>
              <td>
                <div className="col-md text-end">
                  {selectedYear} yil {selectedQuarter} chorak davri uchun
                  ma'lumotlar
                </div>
              </td>
            </tr>
            <tr>
              <th colSpan="12" className="text-center bg-light">
                Davlat ishtirokidagi korxonalarning xatarlari bo'yicha
                umumlashtiruvchi jadval
              </th>
            </tr>
            <tr>
              <th rowSpan="3" className="w-25 text-center align-middle">
                Tashkilotlar
              </th>
      ...
      >
        <Tab eventKey="ytt" title="Yakka tartibdagi tadbirkor">
          <div className="container mt-3 shadow-sm p-4 rounded">
            <h5>Yakka tartibdagi tadbirkor (YTT) to'g'risida ma'lumot</h5>
            <div className="bg-light border p-3 rounded-3">
              ...
              <button
                ...
                onClick={getIndividualInfo}
              >
                <FcSearch />
              </button>
            </div>
          </div>
          <div className="container mt-3">{dispData()}</div>
        </Tab>
        <Tab eventKey="yuridik" title="Yuridik shaxs">
          <div className="container mt-3 shadow-sm p-4 rounded">
            <h5>Yuridik shaxs to'g'risida ma'lumot</h5>
            <div className="bg-light border p-3 rounded-3">
              STIR:
              <input
                type="text"
              ...
              <button
                className="btn btn-sm btn-outline-secondary ms-2"
                onClick={getYuridikInfo}
              >
                <FcSearch />
              </button>
            </div>
          </div>
          <div className="container mt-3">{dispYuridikData()}</div>
        </Tab>
